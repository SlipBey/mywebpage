...
const WorkDetailPage: NextPage<IWorkDetailPage> = ({
	...
	const onApp = async () => {
		...
		await supabase.from("notifications").insert([
			{
				...
				href: `/is-ilanlari/${workplace.id}#basvuranlar`,
		...
		router.push(`/is-ilanlari/${workplace.id}`);
	};
	...
	return (
		<Layout title={`${work.name} Firmasının İş İlanı`}>
			<section className="m-8">
				<div className="w-full bg-gray-300 px-10 py-8 dark:bg-gray-700">
					<div className="mx-auto flex flex-col gap-4">
						...
						<div className="text-md flex flex-col space-y-3 rounded-lg bg-gray-200 py-3 px-4 text-center dark:bg-gray-800 lg:flex-row lg:justify-between lg:space-y-0">
							<div>
							...
							<div>
								...
								<p>{workplace.department}</p>
							</div>
							<div>
								<h1 className="text-gray-700 dark:text-gray-300">
									Eğitim
								</h1>
								<p>{workplace.education}</p>
							</div>
							<div>
								<h1 className="text-gray-700 dark:text-gray-300">
									Askerlik Durumu
								</h1>
								<p>{workplace.solider}</p>
							</div>
							<div>
								<h1 className="text-gray-700 dark:text-gray-300">
									Tecrübe
...
export async function getServerSideProps(context) {
	...
	const { data: work } = await supabase
		.from("workplace")
		.select("*")
		.eq("id", workplace.workplaceId)
		.single();

	const { data: appUser } = await supabase
		.from("applications")
		.select("*")
		.eq("advert", workplace.id)
