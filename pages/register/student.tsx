...
const Register: NextPage<IRegister> = ({ session, classes, users }) => {
	...
	const onStudent = async (e: FormEvent<HTMLFormElement>) => {
		...
		await supabase.from("notifications").insert(
			users.map((u) => ({
				user: u.id,
				text: `${name} ${surname} İsimli öğrenci kayıt olmuş.`,
				title: "Öğrenci Kaydı",
				href: "/admin/kayitlar",
			})),
		);
